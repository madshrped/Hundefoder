...
class BoxPick_pop(Popup):
    def __init__(self, **kwargs):
        ...
        self.num_boxes = 0
        self.title = "Kasse?"
        self.content = BoxLayout(padding = self.width*0.09, orientation="vertical", spacing=self.height*0.09)
        ...
        self.input = RoundedTextInput(bg_color = (.9,.9,1,1), border_buff = 0.05, border_color = (.7,.7, 0.8, 1), input_filter = 'int', multiline = False, size_hint = (0.7,1), input_type = "number")
        self.content.add_widget(self.input)
        self.send = RoundedButton(text = "[b]Send[/b]", default_color = (68/255, 145/255, 68/255, 1), border_color= (1,1,1,1), border_buff= 0.01,  size_hint = (0.7,1))
    ...
    def capture(self, _):
        try: 
            ...
            show_popup("Fejl", f"{self.input.text} er ikke et tal. Kasserne er nummereret, og skal inputtet skal derfor være tal mellem 1 og {self.num_boxes}")
...
class RoundedButton(Button):
    ...
    def __init__(self, **kwargs):
        ...
        pass
    def on_press(self):
        self.button_color = [self.button_color[i] * self.press_shade[i] for i in range(4)]
...
class Scroller(InputContainers):
    ...
    def _update_height(self, *args):
        ...
        self.height = total_height+self.padding[1]*2+self.spacing*len(self.children)
...
def show_popup(_title:str, _text:str = None, sizehint:tuple=(0.75, 0.2), content = None):
    _content = Pop_msg(text = _text) if not content else content
    new = Popup(title=_title, content=_content, size_hint = sizehint)
    new.open()

class event():
    def __init__(self, year:int, month:int, day:int, hour:int, minute:int, dispenser:int=0, package:str=""):
        self.hour_hand = hour
...
class MainWindow(Screen):
    def __init__(self, **kw):
        ...
        self.inputs = [self.year, self.month, self.day, self.hour_hand, self.minut_hand, self.package]
    ...
    def dateCheck(self, date:list):
        ...
        try: 
            datetime(year=date[0], month=date[1], day=date[2], hour=date[3], minute=date[4])
    ...
    def app_update(self, dt):
        for i in range(len(self.events)):
            if self.check_event_relevant(self.events[i]):
                content = f"[b]Inhold[/b]: {self.events[i].package}" if self.events[i].package != '' else ''
                show_popup("Aktivering", f"Aktiverer...\nÅbner for dispenser {self.events[i].dispenser}\n{content}")
    ...
    def add_event(self):
        new_event = event(self.year.text, self.month.text, self.day.text, self.hour_hand.text, self.minut_hand.text, self.dispenser.text, self.package.text)
